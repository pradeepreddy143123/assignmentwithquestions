...
student *insertatpos(student *H,int q) //insert at headposition
{
	int i=1;
	student *T=H;
	student *N=NULL;
...
student *insertbeforekey(student **H,int id)//id means data
{
	int i;
	student *T=*H;
	student *N=NULL;
	N=createnode();
	if(T->id==id)      //here T->id means temporary id the id which we are giving to the position we want to place the id at the position
	{
	...
	else
	{
		while(id != T->next->id)
		{
			T=T->next;     //to return new address and finding before node and it goes to next position
